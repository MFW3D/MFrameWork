...
#define list_for_each_entry_safe(pos, n, head, member)			\
	for (pos = list_entry((head)->next, TypeofHelper<decltype(pos)>::Type, member),	\
		n = list_entry(pos->member.next, TypeofHelper<decltype(pos)>::Type, member);	\
	     &(pos->member) != (head); 					\
	     pos = n, n = list_entry(n->member.next, TypeofHelper<decltype(n)>::Type, member))
#endif
