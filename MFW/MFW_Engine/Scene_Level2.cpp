...
{
	MFW3D::SelectMenu* selectMenu = mTrayMgr->createLongSelectMenu(
	...
	mTrayMgr->createCheckBox(MFW3D::TL_TOPLEFT, "SpinLightButton", "Spin Light", 175)->setChecked(true);

	mTrayMgr->createButton(MFW3D::TL_TOPRIGHT, "PageButtonControl", "Page", 175);

	for (size_t i = 0; i<CONTROLS_PER_PAGE; i++)
	{
		mShaderControls[i] = mTrayMgr->createThickSlider(MFW3D::TL_TOPRIGHT,
...
void Scene_Level2::changePage(int pageNum /* = -1 : toggle */)
{
	...
	static_cast<MFW3D::Button*>(mTrayMgr->getWidget("PageButtonControl"))->setCaption(pageText);
